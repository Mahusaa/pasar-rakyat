...
const CashierCombobox: React.FC<Props> = ({ className, onSelect }) => {
  ...
  return (
    <div className={cn("relative", className)}> 
      ...
      >
        {value ? frameworks.find((framework) => framework.value === value)?.label : 'Kasir'}
        ...
        <div className="absolute ml-10 bottom-full left-0 z-10 w-[150px] bg-white border border-gray-300 rounded-t-md shadow-lg mb-1">
          {frameworks.map((framework) => (
            <div
              key={framework.value}
              className={cn( 
                'relative flex items-center rounded-lg px-2 py-1.5 text-sm outline-none cursor-pointer transition-colors duration-300 text-black hover:bg-gray-100 hover:mx-px',
                { 'bg-gray-100': value === framework.value }
              )}
...
export default CashierCombobox;
